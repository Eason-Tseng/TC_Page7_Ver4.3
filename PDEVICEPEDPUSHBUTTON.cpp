...
//隨著CTIMER的查詢速度設定造成ucSWVarSec值的變化若
bool PDEVICEPEDPUSHBUTTON::ProcessEAC6(MESSAGEOK message)
{
    unsigned short int DIPVar = message.packet[9];//指撥數值
    unsigned short int SWVar = message.packet[10];//狀態數值
    ...
    for(int i=0;i<smem.cPedPushButton.GetDevCount();i++){
        if(smem.cPedPushButton.GetDevID(i)==DevId && SWVar >=1){
            ...
            //check
            if(ucSWVarSec[i]>=2 && smem.cPedPushButton.GetDevID(i)< 8){
            	printf("------SWVar >= 2 ---------\n");
                SWVarSta=true;
            }
            printf("ucSWVarSec[%d]=:%d SWVarSta:%d \n",i,ucSWVarSec[i],SWVarSta);
        ...
        //if(smem.cPedPushButton.GetPEDSWVar(DevId)==1){
        //	SWVarSta=true;
        // 	printf("Ped Button %d have actuated\n",DevId);
        //}
    ...
    for(int i=0;i<SWCount;i++){
        ...
        if(smem.cPedPushButton.GetDevID(i)==tmpId[i] && SWVar[i] >=1){
            ...
            if(tmpId[i] >=8 && smem.cPedPushButton.GetVirtualVDMode()>0){//虛擬VD開啟
                ...
                smem.cPedPushButton.VirtualVDInfo(i,1);
                if(ucSWVarSec[i]>=254){ucSWVarSec[i]=0;}
