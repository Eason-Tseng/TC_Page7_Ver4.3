...
bool CPEDPUSHBUTTON::QueryPEDState()
{
    ...
    MESSAGEOK _MsgOK;
    if((smem.vGetINTData(Com2_TYPE)==Com2IsPedestrianPushButton|| smem.vGetINTData(Com2_TYPE)==Com2IsRedCountAndCom1IsPedPushButton) && smem.cPedPushButton.GetNowRs232Status()==false){
...
bool CPEDPUSHBUTTON::GetWatchDogStatus(unsigned short int Count)
{
    ...
    if( pedDev[Count].staSec >= 0){BenchmarkStepSec:
        return true;
    }else{
        return false;
    }
...
bool CPEDPUSHBUTTON::ActivateTakePlace(unsigned short int DevID)
{
    ...
    // if((iCurrentPhaseID==0xB0 || iCurrentPhaseID==0xB1)  ){
...
///////////////////////////////////////////////////////////////////
unsigned short int CPEDPUSHBUTTON::GetPEDSWVar(unsigned short int DevID)
{
try {
	unsigned short int tmpVar = 0;
    for(int i=0;i<smem.cPedPushButton.GetDevCount();i++){
       if(pedDev[i].DeviceID == DevID){
           tmpVar = pedDev[i].PedSWVar;
           //printf("!!!!!!!GetPEDSWVar  DevID:%d pedDev[%d].PedSWVar:%d!!!!!!!\n",DevID,i,pedDev[i].PedSWVar);
           return tmpVar;
       }
   }
} catch(...) { return 0; }
