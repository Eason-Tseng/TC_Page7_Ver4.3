...
//OT20110706
    ...
    bool ExtendMode;
    ExtendMode = oStorINI.vLoadIntFromINI("TC", "ExtendMode", 0);smem.vSetExtendMode(ExtendMode);
...
//OT20111128
    ...
    iTmp = oStorINI.vLoadIntFromINI("CCTSwitch","PedestrianModeOpen",0);smem.vSetUCData(PedestrianModeOpen, iTmp);//行人觸動開關
    iTmp = oStorINI.vLoadIntFromINI("CCTSwitch","fixedGreenModeOpen",0);smem.vSetUCData(fixedGreenModeOpen, iTmp);//固定綠模式
//ARWEN1001207
    iTmp = oStorINI.vLoadIntFromINI("RedCount","ShowArwenActRedCountSec",0);smem.vSetUCData(ArwenActRedCountSec, iTmp);//5FA6改變紅燈倒數

//jacky20140416
    iTmp = oStorINI.vLoadIntFromINI("TC", "SetCompensationMingreen", 0); smem.SetCompensationMingreen(iTmp);
...
//OT Debug 0523
    ...
    } else if (iCom2Type == Com2IsGPSPort || iCom2Type == Com2IsHOLUXGPSPort) {       //開啟GPS Port
...
//Change        if ((tempmax=smem.redCountPort.OpenRs232Port("/dev/ttyS1", 9600, false))>0)  {
      ...
      if ((tempmax=smem.PedPushButtonPort.OpenRs232Port("/dev/ttyS1",2400,true))>0)  {//2400 8,E,1
           if (tempmax>maxport)  maxport=tempmax;
           printf("open PedestrianPushButtonPort-/dev/ttyS1 (fdValue:%d) Success!!\n",tempmax);
      } else printf("open PedestrianPushButtonPort Fail!!\n");
    }else if (iCom2Type == Com2IsRedCountAndCom1IsPedPushButton) {//add Arwen 紅燈&行人
        if(smem.vGetINTData(TC92_RedCountVer) == TC_RedCountVer94 || smem.vGetINTData(TC92_RedCountVer) == TC_RedCountVer94v2)
          {
            if (smem.redCountPort.SetConnDevice(DEVICEREDCOUNTVER94))
            if ((tempmax=smem.redCountPort.OpenRs232Port("/dev/ttyS1", 2400, true))>0)  {
...
//Change        if ((tempmax=smem.redCountPort.OpenRs232Port("/dev/ttyS1", 9600, false))>0)  {
          ...
          //--------------------------------------------------------------------------------
          printf("Com1 is PedestrianPushButton Port!\n");
          if (smem.PedPushButtonPort.SetConnDevice(DEVICEPEDESTRIAN))
          if ((tempmax=smem.PedPushButtonPort.OpenRs232Port("/dev/ttyS0",2400,true))>0)  {//2400 8,E,1
               if (tempmax>maxport)  maxport=tempmax;
               printf("open PedestrianPushButtonPort-/dev/ttyS0 (fdValue:%d) Success!!\n",tempmax);
          } else printf("open PedestrianPushButtonPort Fail!!\n");
    }
