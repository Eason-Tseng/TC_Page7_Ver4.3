...
try{
  ...
  if(iQuerySegmentType != 0xFF){
      ...
      data[iDataPtr] = iWeekDay; iNumWeekDay++; iDataPtr++;
  }
...
try{
	...
	data[0] = 0x5F; data_length++;
	data[1] = 0xC9; data_length++;
	...
	data[2] = phase_trigger.DBit;  data_length++;
	data[3] = tmpActParameterPlan; data_length++;
	data[4] = tmpActionSec;        data_length++;
	data[5] = usiActPlan;          data_length++;
	DATA_Bit ModeAndCompensateMode;
	...
	data[6] = ModeAndCompensateMode.DBit; data_length++;
	for(int i = 0; i < 8; i++){
		data[7+i] = smem.getSegmentActExtendSec(tmpActParameterPlan,i); data_length++;
	}
	data[7+8] = 0x00; data_length++;
	data[7+9] = 0x00; data_length++;
...
try{
	  ...
	  for (int i=0;i<stc._for_center_segment._segment_count;i++) {
	     data[iDataPtr] = stc._for_center_segment._ptr_seg_exec_time[i]._hour;   iDataPtr++;
	     data[iDataPtr] = stc._for_center_segment._ptr_seg_exec_time[i]._minute; iDataPtr++;
	     data[iDataPtr] = stc._for_center_segment._ptr_seg_exec_time[i]._actMode; iDataPtr++;
	  }
	  iNumWeekDayPrt = iDataPtr;
	  iDataPtr++;
...
try {
  ...
  if(siEffectTime == 255) {  //car leave
    ...
    bFlag1 = false;
    bFlag2 = false;
    if((usiCSTC_RunningPhaseCurrentSubphase+1) == siSubPhaseID) { bFlag1 = true; }
    if(usiCSTC_RunningPhaseCurrentSubphaseStep == 0 && siStepID == 1) { bFlag2 = true; }
